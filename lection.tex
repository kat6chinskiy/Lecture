...
То есть как было сказано выше причина не в вязкой диссипацие, а в рассеивание.
 ...
 Где эти два слагаемых являются равновестными значениями.
...
слагаемое $ \frac{\partial \rho_{0}}{\partial t} = 0 $, а $ div( \rho'\vec{v}') $ нелинейна и мала по сравнению с $ div( \rho_{0}\vec{v}) $. То есть в итоге получили следующее выражение:
...
Это одно из основопологающее отличие от электромагнитной волны, потому что она поперечная.
...
Вспомним, что теплоемкость это энергия которая необходима для того чтобы нагреть тело на $ 1 $ градус. 
...
\end{equation}
где $ \gamma $ - отношение теплоемкостей.
...
И это всё при $ x = 0 $.
\begin{equation*}
\frac{cos(\:\vartheta_{1}\:)}{c_{1}}\bracket{A_{1}-A_{1}'}= \frac{cos(\:\vartheta_{2}\:)}{c_{1}}A_{2}
\end{equation*}
